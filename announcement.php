        ...
        <?php
          ...
          if($total_rows>0){
            while($post=mysqli_fetch_array($all_posts_results)){
              ...
              echo '<p id="header"><b>Ανακοίνωση ';echo $post["id"];echo'</b><form action="deleteannouncement.php" method="post"><button class="tutor_edit" type="submit"><input type="hidden" name="postID" value=';echo $post['id'];echo '>Διαγραφή</button></form>';echo '<form action="editannouncement.php" method="post"><button class="tutor_edit" type="submit"><input type="hidden" name="postID" value=';echo $post['id'];echo '>Επεξεργασία</button></form>';
